...
{
    public partial class ListFournisseur : System.Web.UI.Page
    {
        string nom_;
        public void EnregistrerTrace()
        {
            #region Enregistrer trace
            TraceSystem tr = new TraceSystem(nom_, "Lister fournisseur", DateTime.Now.ToString());
            int result = 0;
            try
            {
        ...
        public void Verifier_Roles()
        {
            ...
            try
            {
                util = UtilisateurControleur.Verifier_Roles(nom_, "ListFournisseur.aspx");
                if (util != null)
                {
        ...
        {
            Response.Redirect("AddFournisseur.aspx");
        }
    }
    }